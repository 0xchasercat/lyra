...
export class GitPipeline {
    ...
    try {
      ...
      for (const workspace of preview.workspaces) {
        ...
        if (merged.exitCode !== 0) {
          ...
          if (this.#mode === "auto" && this.#options.resolver && await this.#options.resolver.resolve({ repo: stagingPath, conflict, allWorkspaces: preview.workspaces }, signal)) {
            const unresolved = (await this.git(stagingPath, ["diff", "--name-only", "--diff-filter=U"], signal)).stdout.trim();
            if (unresolved.length > 0) return { ok: false, preview: selectedName, snapshot, conflicts: [conflict], message: `Resolver left unresolved conflicts: ${unresolved}.` };
            ...
            if (committed.exitCode !== 0) return { ok: false, preview: selectedName, snapshot, conflicts: [conflict], message: `Resolver changes could not be committed: ${committed.stderr.trim()}.` };
          } else {
            return { ok: false, preview: selectedName, snapshot, conflicts: [conflict], message: `Conflicts in ${files.join(", ") || "unknown files"}; preview retained for manual resolution.` };
          }
        }
      ...
      this.log("apply", true, `applied preview ${selectedName} at ${head}; snapshot ${snapshot.name}`);
      return { ok: true, preview: selectedName, snapshot, appliedHead: head, message: `Applied ${preview.workspaces.length} workspace(s); rollback snapshot ${snapshot.name}.` };
    } finally { await rm(stagingPath, { recursive: true, force: true }); }
  }

  async snapshot(name = `snapshot-${timestampName(this.#now())}`, signal?: AbortSignal): Promise<SnapshotRecord> {
    await this.initialize(signal);
    ...
    const head = (await this.git(this.origin, ["rev-parse", "HEAD"], signal)).stdout.trim();
    const branchOutput = await this.git(this.origin, ["symbolic-ref", "--short", "HEAD"], signal);
  ...
  private async requireCleanOrigin(operation: string, signal?: AbortSignal): Promise<void> {
    ...
    if (status.stdout.trim().length > 0) throw new Error(`Origin has tracked or untracked changes; commit, move, or restore them before ${operation} so rollback remains exact.`);
  }
