...
export class McpStdioClient {
  ...
  async request(method: string, params?: unknown, signal?: AbortSignal): Promise<unknown> {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => { const pending = this.#pending.get(id); if (!pending) return; this.#pending.delete(id); pending.cleanup?.(); const error = new McpError("timeout", `MCP request ${method} exceeded ${timeoutMs}ms.`); pending.reject(error); this.fail(error); }, timeoutMs);
      const cleanup = (): void => signal?.removeEventListener("abort", onAbort);
      const onAbort = (): void => { const pending = this.#pending.get(id); if (!pending) return; this.#pending.delete(id); clearTimeout(pending.timer); pending.reject(signal?.reason); this.fail(signal?.reason); };
      this.#pending.set(id, { method, resolve, reject, timer, cleanup });
      signal?.addEventListener("abort", onAbort, { once: true });
      this.#write(message).catch((error) => { const pending = this.#pending.get(id); if (!pending) return; this.#pending.delete(id); clearTimeout(pending.timer); pending.cleanup?.(); pending.reject(new McpError("transport", String(error))); });
    });
  }
  ...
  async #write(value: string): Promise<void> { this.#writeTail = this.#writeTail.then(async () => { if (this.#closed) throw new McpError("closed", `MCP server ${this.name} is closed.`); const stdin = this.#process.stdin; if (stdin === undefined || typeof stdin === "number") throw new McpError("transport", `MCP server ${this.name} stdin is unavailable.`); await stdin.write(value); }); return this.#writeTail; }
  ...
  private handleLine(line: string): void { let message: unknown; try { message = JSON.parse(line); } catch { this.fail(new McpError("protocol", `MCP server ${this.name} emitted invalid JSON.`)); return; } if (!message || typeof message !== "object" || typeof (message as { id?: unknown }).id !== "number") return; const id = (message as { id: number }).id; const pending = this.#pending.get(id); if (!pending) return; this.#pending.delete(id); clearTimeout(pending.timer); pending.cleanup?.(); const value = message as { result?: unknown; error?: { code?: unknown; message?: unknown } }; if (value.error) { pending.reject(new McpError(typeof value.error.code === "number" ? value.error.code : "server", typeof value.error.message === "string" ? value.error.message : "MCP server request failed.")); return; } pending.resolve(value.result); }
